...
{
    ROS_DEBUG_STREAM("CalGuessPose start");
    _guess_pose.pos = _prev_pose.pos + (_scan_duration/10e3) * _guess_offset.vel + 0.5 * pow((_scan_duration/10e3), 2) * _guess_offset.acc;
...
void LidarMapping::CalDiffPose(Pose6D &_ndt_pose, Pose6D &_prev_pose, double _scan_duration, DiffVAW &_guess_offset)
{
    ...
    _guess_offset.acc = 2 / (pow((_scan_duration/10e3), 2)) * (_ndt_pose.pos - _prev_pose.pos - (_guess_offset.vel * (_scan_duration/10e3)));
    _guess_offset.vel = (_ndt_pose.pos - _prev_pose.pos) / (_scan_duration/10e3);
...
void LidarMapping::InitROS()
{
    ...
    cloud_sub = nh.subscribe(cloud_input, 10000, &LidarMapping::CloudCallback, this);
